...
function obtenerColaboradores() {
  ...
  return hoja.getDataRange().getValues()
    ...
    .map((fila, i) => ({
      ...
      ubicacion: fila[4] ? fila[4].toString() : '',
      rowIndex: i + 2
    }))
    .filter(colaborador => colaborador.estado === 'activo');
}

function obtenerUbicacionColaborador(identificacion) {
  const idBuscado = identificacion ? identificacion.toString().trim() : '';
  if (!idBuscado) return '';

  const hoja = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Colaboradores');
  const colaborador = hoja.getDataRange().getValues().slice(1).find(fila =>
    fila[1] && fila[1].toString().trim() === idBuscado
  );

  return colaborador && colaborador[4] ? colaborador[4].toString() : '';
}
...
function mapearEquipo(fila, indice, mapaColaboradores = {}) {
  ...
  const ubicacionEquipo = fila[1] ? fila[1].toString() : '';
  ...
  return datos
    ...
    .filter(equipo => {
      if (tipoActa === 'recepcion') {
        return equipo.colaborador === idBuscado && equipo.disponibilidad_equipo === 'ocupado';
      }
      if (tipoActa === 'cambio') {
        return (equipo.colaborador === '' || equipo.colaborador === SISTEMAS_ADMIN_ID) &&
...
function reemplazarPrimeraCoincidencia(body, marcador, valor) {
  ...
  const fin = inicio + marcador.length - 1;
...
function generarActa(data) {
  ...
  if (data.usuario) {
    data.usuario.ciudad = obtenerUbicacionColaborador(data.usuario.id);
  }
  ...
  reemplazarTexto(body, '{{ciudad}}', data.usuario.ciudad || '');
...
function actualizarInventario(data) {
  ...
  (data.equipos || []).forEach(eq => {
    ...
    if (data.tipoActa === 'recepcion') {
      hojaEquipos.getRange(eq.rowIndex, 1).setValue(SISTEMAS_ADMIN_ID); // colaborador a bodega
      hojaEquipos.getRange(eq.rowIndex, 2).setValue(''); // ubicacion_equipo vacía en bodega
    ...
    hojaEquipos.getRange(eq.rowIndex, 1).setValue(data.usuario.id); // colaborador al colaborador
    hojaEquipos.getRange(eq.rowIndex, 2).setValue(data.usuario.ciudad || ''); // ubicacion_equipo del colaborador
...
function actualizarInventarioCambio(data, hojaEquipos, hojaAccesorios) {
  ...
  if (equipoAnterior.rowIndex) {
    ...
    hojaEquipos.getRange(equipoAnterior.rowIndex, 2).setValue('');
  ...
  if (equipoNuevo.rowIndex) {
    ...
    hojaEquipos.getRange(equipoNuevo.rowIndex, 2).setValue(data.usuario.ciudad || '');
